 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
const z = globalThis, n1 = (t) => t, I = z.trustedTypes, c1 = I ? I.createPolicy("lit-html", { createHTML: (t) => t }) : void 0, y1 = "$lit$", v = `lit$${Math.random().toFixed(9).slice(2)}$`, A1 = "?" + v, N1 = `<${A1}>`, k = document, T = () => k.createComment(""), Z = (t) => t === null || typeof t != "object" && typeof t != "function", t1 = Array.isArray, D1 = (t) => t1(t) || typeof (t == null ? void 0 : t[Symbol.iterator]) == "function", W = `[ 	
\f\r]`, O = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, l1 = /-->/g, d1 = />/g, y = RegExp(`>|${W}(?:([^\\s"'>=/]+)(${W}*=${W}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), h1 = /'/g, C1 = /"/g, w1 = /^(?:script|style|textarea|title)$/i, R1 = (t) => (e, ...i) => ({ _$litType$: t, strings: e, values: i }), l = R1(1), L = Symbol.for("lit-noChange"), h = Symbol.for("lit-nothing"), u1 = /* @__PURE__ */ new WeakMap(), w = k.createTreeWalker(k, 129);
function x1(t, e) {
...
const V1 = (t, e) => {
  ...
  for (let c = 0; c < i; c++) {
    const n = t[c];
    let C, u, d = -1, g = 0;
    for (; g < n.length && (a.lastIndex = g, u = a.exec(n), u !== null); ) g = a.lastIndex, a === O ? u[1] === "!--" ? a = l1 : u[1] !== void 0 ? a = d1 : u[2] !== void 0 ? (w1.test(u[2]) && (s = RegExp("</" + u[2], "g")), a = y) : u[3] !== void 0 && (a = y) : a === y ? u[0] === ">" ? (a = s ?? O, d = -1) : u[1] === void 0 ? d = -2 : (d = a.lastIndex - u[2].length, C = u[1], a = u[3] === void 0 ? y : u[3] === '"' ? C1 : h1) : a === C1 || a === h1 ? a = y : a === l1 || a === d1 ? a = O : (a = y, s = void 0);
    const m = a === y && t[c + 1].startsWith("/>") ? " " : "";
    r += a === O ? n + N1 : d >= 0 ? (o.push(C), n.slice(0, d) + y1 + n.slice(d) + v + m) : n + v + (d === -2 ? c : m);
  }
    ...
    for (; (s = w.nextNode()) !== null && n.length < c; ) {
      if (s.nodeType === 1) {
        if (s.hasAttributes()) for (const d of s.getAttributeNames()) if (d.endsWith(y1)) {
          const g = u[a++], m = s.getAttribute(d).split(v), R = /([.?@])?(.*)/.exec(g);
          n.push({ type: 1, index: r, name: R[2], strings: m, ctor: R[1] === "." ? I1 : R[1] === "?" ? B1 : R[1] === "@" ? Q1 : F }), s.removeAttribute(d);
...
class N {
  ...
  _$AI(e, i = this) {
    e = P(this, e, i), Z(e) ? e === h || e == null || e === "" ? (this._$AH !== h && this._$AR(), this._$AH = h) : e !== this._$AH && e !== L && this._(e) : e._$litType$ !== void 0 ? this.$(e) : e.nodeType !== void 0 ? this.T(e) : D1(e) ? this.k(e) : this._(e);
  }
  O(e) {
...
let f = class extends E {
  ...
  vehicleVisual(t) {
    return this.config.image_mode === "custom" && this.config.vehicle_image ? l`<img src=${this.config.vehicle_image} alt=${this.t("vehicle")} loading="lazy" />` : ne(this.model(), this.paintColor(), {
      charging: t.charging,
    ...
    return l`<ha-card class=${this.config.animate === !1 ? "reduce-motion" : ""}>
      <header>
        <div>
          ...
          ${this.config.show_model !== !1 ? l`<p>${se(this.model())}</p>` : h}
        </div>
...
var fe = Object.defineProperty, me = Object.getOwnPropertyDescriptor, D = (t, e, i, o) => {
